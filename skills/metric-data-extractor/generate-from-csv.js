...
// 读取现有CSV
const metricsCsvContent = fs.readFileSync('/home/linxiankun/huawei-ad-ontology/skills/metric-data-extractor/config/metrics.csv', 'utf8');
const dimensionsCsvContent = fs.readFileSync('/home/linxiankun/huawei-ad-ontology/skills/metric-data-extractor/config/dimensions.csv', 'utf8');

const metricsData = Papa.parse(metricsCsvContent, { header: true }).data;
const dimensionsData = Papa.parse(dimensionsCsvContent, { header: true }).data;
...
const metricAliases = {
  ...
  'recallSumCount': '召回量,召回参与量',
  'recallSumWinRate': '召回胜出率,召回率',
  'roughRowParticipation': '粗排总量,粗排参与量',
  'roughRowWinRate': '粗排胜出率,粗排率',
  'preciseRowParticipation': '精排参与量,精排总量',
  'preciseRowWinRate': '精排胜出率,精排率',
  'adFillRateCount': '填充率,填充率次数',
  'adImpRateCount': '展示率,展示率次数',
  'deepCvr': '深层转化率,深层CVR',
  'deepCtcvr': '深层CTCVR,深层点击转化率',
};
...
// 生成增强的 metrics.csv
const metricsRows = ['metric_code,metric_name,metric_desc,category_level1,category_level2,supported_dimensions,metric_aliases,indicator_id,decimal_places'];

metricsData.forEach(row => {
  if (!row.metric_code) return;

  const code = row.metric_code;
  const name = row.metric_name;
  const desc = generateMetricDesc(code, name);
  ...
  const decimalPlaces = row.decimal_places || 8;

  metricsRows.push(`${code},${name},"${desc}",${category[0]},${category[1]},"${dimensions}","${aliases}",${indicatorId},${decimalPlaces}`);
});
...
const dimensionAliases = {
  ...
  'corpId': '广告主ID,广告主id',
  'adGroupId': '任务ID,任务id',
  'campaignId': '计划ID,计划id',
  'dspId': 'DSP_ID,dsp_id',
  'dspName': 'DSP名称,dsp',
  'sspName': 'SSP名称,ssp',
};

function generateDimensionDesc(code, name) {
  ...
  if (type === 'enum') {
    return `${name}，枚举类维度，值固定`;
  } else if (type === 'semi-enum') {
    return `${name}，半枚举类维度，常见值有限但可扩展`;
  } else if (type === 'date') {
...
const dimensionsRows = ['dimension_code,dimension_name,dimension_en_name,dimension_desc,dimension_aliases,value_examples,dimension_type'];

dimensionsData.forEach(row => {
  if (!row.dimension_code) return;

  const code = row.dimension_code;
  const name = row.dimension_name;
  const enName = row.dimension_en_name;
  const desc = generateDimensionDesc(code, name);
  ...
  dimensionsRows.push(`${code},${name},${enName},"${desc}","${aliases}","${examples}",${type}`);
});

// 写入文件
fs.writeFileSync('/home/linxiankun/huawei-ad-ontology/skills/metric-data-extractor/config/metrics.csv', metricsRows.join('\n'), 'utf8');
fs.writeFileSync('/home/linxiankun/huawei-ad-ontology/skills/metric-data-extractor/config/dimensions.csv', dimensionsRows.join('\n'), 'utf8');

console.log('✓ 生成 metrics.csv 完成，共', metricsRows.length - 1, '个指标');
console.log('✓ 生成 dimensions.csv 完成，共', dimensionsRows.length - 1, '个维度');
