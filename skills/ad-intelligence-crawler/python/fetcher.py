...
class Crawl4AiFetcher(BaseFetcher):
    ...
    def __init__(
        ...
        self._run_config = CrawlerRunConfig(
            ...
            wait_until="networkidle",  # 等待网络空闲，确保 SPA 页面 JS 渲染完成
    ...
    async def fetch_raw(self, urls: list[str]) -> list:
        """抓取 URL 并返回原生 CrawlResult 对象（用于链接提取）。

        与 fetch_all 不同，返回引擎原生响应而非 ArticleInsight。
        失败的 URL 会被跳过。
        """
        results = []
        async with AsyncWebCrawler(config=self._browser_config) as crawler:
                ...
                async with self._semaphore:
                    try:
                        ...
                        if result.success:
                            ...
                            logger.warning("crawl4ai 列表页抓取失败 [%s]: %s", url, result.error_message)
                    except Exception as exc:
                        logger.error("crawl4ai 列表页抓取异常 [%s]: %s", url, exc)
        return results
